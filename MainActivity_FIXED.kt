...
class MainActivity : AppCompatActivity() {
    ...
    private val screenCaptureLauncher = registerForActivityResult(
        ...
        if (result.resultCode == Activity.RESULT_OK && result.data != null) {
            val resultCodeValue = result.data?.getIntExtra(ScreenCapturePermissionActivity.EXTRA_RESULT_CODE, -1) ?: -1
            val resultData = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                result.data?.getParcelableExtra(ScreenCapturePermissionActivity.EXTRA_DATA, Intent::class.java)
            } else {
                @Suppress("DEPRECATION")
                result.data?.getParcelableExtra(ScreenCapturePermissionActivity.EXTRA_DATA)
            }
            val monitorIp = etMonitorIp.text.toString()

            if (resultCodeValue != -1 && resultData != null) {
                startMonitoringService(monitorIp, resultCodeValue, resultData)
            }
        } else {
    ...
    private fun startMonitoringService(monitorIp: String, resultCodeValue: Int, data: Intent) {
        val serviceIntent = Intent(this, ConnectionService::class.java).apply {
            ...
            putExtra(ConnectionService.EXTRA_RESULT_CODE, resultCodeValue)
    ...
    private fun handleTouchEvent(eventType: Int, x: Float, y: Float) {
        when (eventType) {
            0 -> {
                touchOverlay?.updateCursorPosition(x, y)
                touchOverlay?.setCursorPressed(false)
            }
            1 -> {
                touchOverlay?.updateCursorPosition(x, y)
                touchOverlay?.setCursorPressed(true)
            }
            2 -> {
                touchOverlay?.updateCursorPosition(x, y)
                touchOverlay?.setCursorPressed(false)
